<?php
    ...
    if (!isset($_SESSION['board'])) {
        if (isset($_ENV["TESTING"])) {
            echo "Invalid include of index.php";
            exit(1);
        }
...
?>
    ...
    <form method="post" action="game/play.php">
        <select name="piece">
            <?php
            foreach (getHand($hand[$player]) as $tile) {
    ...
    <ol>
        <?php
        $db = new DBHandeler();
        $stmt = $db->getCon()->prepare('SELECT * FROM moves WHERE game_id = ' . $_SESSION['game_id']);
        $stmt->execute();
...
{
    foreach ($hand as $name => $available) {
        if ($available) {
            yield $name;
        }
    }
